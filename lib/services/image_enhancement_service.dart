...
class ImageEnhancementService {
  ...
  /// Normalización de brillo robusta usando percentiles
  /// Ignora píxeles extremos para mejor resultado
  static img.Image _normalizeBrightness(img.Image image) {
    ...
    final robustMean = gray
        .where((v) => v >= p5 && v <= p95)
        .fold<int>(0, (a, b) => a + b) ~/ 
        (gray.where((v) => v >= p5 && v <= p95).length);
    ...
    for (int y = 0; y < image.height; y++) {
      for (int x = 0; x < image.width; x++) {
        ...
        final r = (px.r.toInt() + adjustment).clamp(0, 255);
        final g = (px.g.toInt() + adjustment).clamp(0, 255);
        final b = (px.b.toInt() + adjustment).clamp(0, 255);
        result.setPixelRgba(x, y, img.Pixel(r, g, b, px.a.toInt()));
      }
    }
    return result;
  }
    ...
    for (int y = 0; y < original.height; y++) {
      for (int x = 0; x < original.width; x++) {
        if (idx < gray.length) {
          ...
          result.setPixelRgba(x, y, img.Pixel(value, value, value, 255));
        }
      }
    }
    return result;
